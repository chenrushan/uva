...
void
DFS(struct bitflag *bf, int idx)
{
        /* very very BAD trick, but it's on-line judge
         */
        char bflg[sizeof(struct bitflag) + MAX_STR_LEN / 8 + 1];
        struct bitflag *lbf = (struct bitflag *)bflg; /* local bit flag */
        char lstc = 0;
        ...
        /* very very BAD trick
         */
        memcpy(bflg, bf, sizeof(bflg));
        lbf->bits = bflg + sizeof(struct bitflag);

        while (1) {
                int pos = bitflag_first0(lbf);
                if (pos == -1)
                        break;
